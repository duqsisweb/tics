            ...
            <tbody>
                <?php foreach ($arr as $fila) { ?>
                    ...
                    <td class="hidden-cell"><?php echo $Usua_asigna; ?></td>
                        ...
                        <td>
                            <textarea placeholder="AGREGE UNA OBSERVACIÓN" id="observaciones_desasigna_sim<?= $fila['id'] ?>" name="observaciones_desasigna_sim" style="width: 300px; height: 160px;"></textarea>
                        </td>

                        <td>
                            <input id="link_sim_desasigna<?= $fila['id'] ?>" name="link_sim_desasigna"></input>
                        </td>

                        <td>
                            <button id="enviarsimcard" style="display: none;" type="submit" class="btn btn-outline-danger asignar-btn" 
                            data-id="<?= $fila['id'] ?>" 
...
<script>
    $(document).ready(function() {
        $('.asignar-btn').on('click', function() {
            ...
            var observaciones_desasigna_sim = $('#observaciones_desasigna_sim' + id).val();
            var link_sim_desasigna = $('#link_sim_desasigna' + id).val();
            ...
            console.log("observaciones de Asignacion", observaciones_desasigna_sim);
            console.log("link Drive", link_sim_desasigna);

            $.ajax({
                url: 'historial_remover_asignacion/deletesimcardhistorial.php',
                type: 'POST',
                data: {
                    ...
                    observaciones_desasigna_sim: observaciones_desasigna_sim,
                    link_sim_desasigna: link_sim_desasigna
                },
