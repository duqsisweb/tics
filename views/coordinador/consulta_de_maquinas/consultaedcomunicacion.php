...
if (isset($_POST['tipo_edcomunicacion'])) {

    $tipo_edcomunicacion = $_POST['tipo_edcomunicacion'];
    ...
    where estado = '6' AND [descripcion_edcomunicacion]='$tipo_edcomunicacion' ");
                ...
                <tbody>
                    <?php foreach ($arr as $fila) { ?>
                        <tr>
                            ...
                            <td>
                            <textarea placeholder="AGREGE UNA OBSERVACIÓN" name="observaciones_asigna_edc" id="observaciones_asigna_edc<?= $fila['id'] ?>" style="width: 300px; height: 160px;"></textarea>
                            </td>

                            <td>
                                <input id="link_edc_asigna<?= $fila['id'] ?>" name="link_edc_asigna"></input>
                            </td>
                            <td>
                                <button id="enviaredcomunicacion" style="display: none;" type="submit" class="btn btn-outline-warning asignar-btn" 
                                data-id="<?= $fila['id'] ?>" 
    ...
    <script>
        $(document).ready(function() {
            $('.asignar-btn').on('click', function() {
                ...
                var observaciones_asigna_edc = $('#observaciones_asigna_edc' + id).val();
                var link_edc_asigna = $('#link_edc_asigna' + id).val();
                ...
                console.log("observaciones de Asignacion", observaciones_asigna_edc);
                console.log("link Drive", link_edc_asigna);

                $.ajax({
                    url: 'asignar_maquinas/insertaredcomunicacion.php',
                    type: 'POST',
                    data: {
                        ...
                        observaciones_asigna_edc: observaciones_asigna_edc,
                        link_edc_asigna: link_edc_asigna
                    },
            ...
            $('.showAlertButton').click(function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        // Cerrar el modal
                        $('#modaledcomunicacion').modal('hide');
                        //mensaje de exito
                        Swal.fire('¡Guardado!', '', 'success');

                        // Hacer el update mediante AJAX con el ID obtenido para actualizar
                        $.ajax({
                            url: 'update/actualizaredcomunicacion.php',
                            type: 'POST',
                            data: {
