            ...
            <tbody>
                <?php foreach ($arr as $fila) { ?>
                    ...
                    <td class="hidden-cell"><?php echo $Usua_asigna; ?></td>
                        ...
                        <td>
                            <textarea placeholder="AGREGE UNA OBSERVACIÓN" id="observaciones_asigna_sim<?= $fila['id'] ?>" name="observaciones_asigna_sim" style="width: 300px; height: 160px;"></textarea>
                        </td>

                        <td>
                            <input id="link_sim_asigna<?= $fila['id'] ?>" name="link_sim_asigna"></input>
                        </td>
...
<script>
    $(document).ready(function() {
        $('.asignar-btn').on('click', function() {
            ...
            var observaciones_asigna_sim = $('#observaciones_asigna_sim' + id).val();
            var link_sim_asigna = $('#link_sim_asigna' + id).val();
            ...
            console.log("observaciones de Asignacion", observaciones_asigna_sim);
            console.log("link Drive", link_sim_asigna);

            $.ajax({
                url: 'asignar_maquinas/insertarsimcard.php',
                type: 'POST',
                data: {
                    ...
                    observaciones_asigna_sim: observaciones_asigna_sim,
                    link_sim_asigna: link_sim_asigna
                },
